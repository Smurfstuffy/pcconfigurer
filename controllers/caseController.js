...
const searchCases = async (req, res) => {
  ...
  try {
    ...
    const totalProducts = await Case.countDocuments({ name: { $regex: regex } });

    const totalPages = Math.ceil(totalProducts / limit);

    const cases = await Case.find({ name: { $regex: regex } })
      .skip((page - 1) * limit)
      .limit(limit);
    ...
    res.status(500).json({ message: 'Error searching cases', error });
  }
};

module.exports = {getCases, getCaseById, updateCasesWithImageUrls, searchCases};