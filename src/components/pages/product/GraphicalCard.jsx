...
const GraphicalCard = observer(() => {
  ...
  useEffect(() => {
    if (user) {
      (async () => {
        try {
          await fetchData(
            'http://localhost:8080/api/graphicalcard',
            'post',
            {
  ...
  const handleAdd = () => {
    PCConfig.setGraphicalCard(data);
            ...
            <div className="flex flex-col border-gray-500">
              <div className="flex flex-col h-full">
                ...
                <div className="flex pl-2 py-2">
                  <p className="basis-1/2 text-lg md:text-2xl font-semibold">Chipset:</p>
                  <p className="basis-1/2 text-lg md:text-2xl font-semibold">{data.chipset}</p>
                </div>
                <div className="flex pl-2 py-2">
                  <p className="basis-1/2 text-lg md:text-2xl font-semibold">Memory:</p>
                  <p className="basis-1/2 text-lg md:text-2xl font-semibold">{data.memory}</p>
                </div>
                <div className="flex pl-2 py-2">
                ...
                <div className="flex pl-2 py-2">
                  <p className="basis-1/2 text-lg md:text-2xl font-semibold">Length:</p>
                  <p className="basis-1/2 text-lg md:text-2xl font-semibold">{data.length}</p>
                </div>
                <div className="flex pl-2 py-2">
              ...
              <div className="flex p-2 ">
                {isObjEmpty(PCConfig.config.graphicalCard) ?
                  <button className="primary-button text-center w-full" onClick={() => handleAdd()}>Add Graphical Card</button> :
                  PCConfig.config.graphicalCard.name === data.name ?
                    <button className="neutral-button text-center w-full" onClick={() => PCConfig.removeGraphicalCard()}>Remove Graphical Card</button> :
                    <button className="primary-button text-center w-full" onClick={() => handleAdd()}>Add Graphical Card</button>
                }
              </div>
            </div>
